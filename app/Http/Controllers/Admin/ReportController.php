...
class ReportController extends Controller
{
    ...
    public function indexTeachingMaterial()
    {
        $types = Type::all();
        return view('admin.reports.teaching-mats.index', compact('types'));
    }
    ...
    public function exportGoodExcel2(Request $req)
    {
        ...
        return Excel::download(new GoodsExport2($bigData), 'ExportExcel.xlsx');
    }
    ...
    public function showMatReport(Request $req){
        ...
        $sum = Material::where('type_id', 2)->select(DB::raw('sum(amount * price_unit) as total'))->first();
    ...
    public function showTeachingMatReport(Request $req)
    {
        ...
        $sum = Material::where('type_id', 1)->select(DB::raw('sum(amount * price_unit) as total'))->first();
        ...
        if ($req->type == 'excel') {
            return view('admin.reports.teaching-mats.show', compact('bigData'));
        } else {
            $pdf = PDF::loadView('admin.reports.teaching-mats.export', compact('bigData'));
            $pdf->setPaper('A4', 'landscape');
            return $pdf->stream('report.pdf'); //แบบนี้จะ stream มา preview
        }
    ...
    public function exportTeachingMatExcel(Request $req)
    {
        $bigData = json_decode($req->bigData, true);
    ...
    public function showTeachingMatReport2(Request $req)
    {
        ...
        if ($req->type == 'excel') {
            return view('admin.reports.teaching-mats.show2', compact('bigData'));
        } else {
            $pdf = PDF::loadView('admin.reports.teaching-mats.export2', compact('bigData'));
            $pdf->setPaper('A4', 'landscape');
            return $pdf->stream('report.pdf'); //แบบนี้จะ stream มา preview
        }
    ...
    public function exportTeachingMatExcel2(Request $req)
    {
        $bigData = json_decode($req->bigData, true);
